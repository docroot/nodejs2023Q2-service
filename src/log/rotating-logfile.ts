...
export class RotatingLogfile {
    ...
    constructor(private readonly dir: string, private readonly filename: string, private readonly maxSize: number) {
    ...
    private async processQueue() {
       ...
       // while(this.queue.length > 0 ) {
            ...
            this.count ++;
            if(this.count % this.checkInterval === 0 ) { // checking the log file for a need of rotation by every message is too often
    ...
    private async rotateLog(): Promise<boolean> {
        try {
            ...
            filtered.forEach((file) => {
                let num = this.getFileIndex(file);
            ...
            for(let i = idxs.length - 1; i >= 0; i-- ) {
                if( i > 0 && idxs[i - 1] === ( idxs[i] - 1) || i === 0 && idxs[i] === 0 ){ // there is a logfile with previous index.
                    ...
                    if(idxs[i] === 9 ) {
                        ...
                        const filepath1 = path.join(this.dir, this.filename + '.' + (idxs[i] + 1));
